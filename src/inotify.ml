...
external add_watch   : Unix.file_descr -> string -> ev_req list -> wd = "stub_inotify_add_watch"
external rm_watch    : Unix.file_descr -> wd -> unit = "stub_inotify_rm_watch"

external convert     : string -> (wd * ev list * int32 * int) = "stub_inotify_convert"
...
let read fd =
	...
	while !i < toread
	do
	    let wd,evs,cookie,len = convert (String.sub buf !i ss) in
	    let s =
		if len > 0 then
		    ...
		    None
	    in
	    ret := {wd=wd; evs=evs; cookie=cookie; name=s} :: !ret;
	    i := !i + (ss + len)
	done;
